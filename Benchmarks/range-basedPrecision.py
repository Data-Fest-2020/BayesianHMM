# -*- coding: utf-8 -*-
"""
Created on Sun Sep 20 13:29:48 2020
...
class TSMetric:
    ...
    def score(self, values_real, values_predicted):
        ...
        # store the original arrays as y_hat and y_true, in order to be able to reuse the original array with multiclass
        y_hat = values_predicted
        y_true = values_real
    
        # define results table
        classes_pred = np.unique(y_hat)
        classes_real = np.unique(y_true)
        classes = set(classes_pred).intersection(set(classes_real))
        classes = list(classes)
         
        num_classes = len(classes)
        A = np.random.randint(0, 1, size=3*num_classes).reshape(num_classes, 3)
        head = ['precision','recall','f1']
        results = pd.DataFrame(A, index=classes, columns= head)

        # transform into binary (one vs. all)
        for j in range(num_classes):
            # transform into binary classification if multiclass
            classe = classes[j]
            values_real = np.where(y_true == classe, 1, 0)
            values_pred = np.where(y_hat == classe, 1, 0)
            
            real_anomalies, predicted_anomalies = self._prepare_data(values_real, values_pred)
            ...
            results.loc[classe] = [precision, recall, Fbeta]
        return results
